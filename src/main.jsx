...
function TopBar({onHome,count,onQuote,updateReady,onApplyUpdate}) { return <header><button className="wordmark" onClick={onHome} aria-label="Blacks Aesthetics home">{/* Real Blacks Aesthetics flat SVG logo insertion point — replace this interim text wordmark only when the approved asset arrives. */}<span>BLACKS</span><small>AESTHETICS</small></button><div className="header-actions">{updateReady ? <button className="update-ready" onClick={onApplyUpdate}>Update ready</button> : null}<button className="quote-button" onClick={onQuote}>Quote <b>{count}</b></button></div></header> }
